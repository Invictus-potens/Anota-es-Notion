...
export async function createNote(req, res) {
  ...
  try {
    ...
    const notion_page_id = await createNotionPage({ title, content, user_id });
    // Salva no MySQL
    const note = await createNoteDB({ user_id, title, content, notion_page_id });
...
export async function updateNote(req, res) {
  ...
  try {
    ...
    if (note.notion_page_id) {
      await updateNotionPage(note.notion_page_id, { title, content });
    }
...
export async function deleteNote(req, res) {
  ...
  try {
    ...
    if (note.notion_page_id) {
      await deleteNotionPage(note.notion_page_id);
    }
